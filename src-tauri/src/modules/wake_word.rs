        ...
        if self.model_status.model_missing {
            ...
            match runtime.run_inference_frame(frame, sample_rate) {
                Ok(result) => {
                    ...
                    let effective_threshold = (manifest_threshold * sensitivity_factor)
                        .clamp(LOCAL_WAKE_WORD_MIN_THRESHOLD, 1.0);
                    ...
                    if self.consecutive_hits >= LOCAL_WAKE_WORD_CONFIRMATION_FRAMES
                        && cooldown_ready
                    {
                        self.last_detection = Some(now);
                        self.consecutive_hits = 0;
